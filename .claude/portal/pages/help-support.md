...
`ib_support_tickets` / `ib:support`, Portal ↔ Admin — see [../../Integration.md](../../Integration.md) §3). The
...
`IBStore`/`fetch` (see [../../Environment-Management-backend.md](../../Environment-Management-backend.md)).
...
- **All contact writes go through `SupportService` → DataSource** — never `window.IBStore.support` or
  ...
  (`support` group, [../../Integration.md](../../Integration.md)).
